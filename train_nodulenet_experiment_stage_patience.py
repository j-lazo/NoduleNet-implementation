...
def parse_args():
    ...
    parser.add_argument(
        '--reset-patience-on-stage-change',
        action=argparse.BooleanOptionalAction,
        default=True,
        help=(
            'Reset the early-stopping patience counter whenever the training stage changes, '
            'e.g. RPN-only -> RPN+RCNN or RPN+RCNN -> RPN+RCNN+mask.'
        ),
    )
...
def main():
    ...
    if args.resume is not None:
        ...
        if history:
            last_row = history[-1]
            previous_stage = (bool(last_row.get('use_rcnn', False)), bool(last_row.get('use_mask', False)))
    ...
    try:
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            current_stage = (bool(net.use_rcnn), bool(net.use_mask))
            stage_changed = previous_stage is not None and current_stage != previous_stage
            if args.reset_patience_on_stage_change and stage_changed:
                epochs_without_improvement = 0
                print(
                    "Training stage changed "
                    f"from use_rcnn={previous_stage[0]}, use_mask={previous_stage[1]} "
                    f"to use_rcnn={current_stage[0]}, use_mask={current_stage[1]}. "
