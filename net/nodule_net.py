...
class MaskHead(nn.Module):
    ...
    def forward(self, detections, features):
        ...
        for detection in detections:
            ...
            # Convert detection coordinates to Python ints for indexing/slicing
            b = int(b)
            z_start = int(round(float(z_start)))
            y_start = int(round(float(y_start)))
            x_start = int(round(float(x_start)))
            z_end = int(round(float(z_end)))
            y_end = int(round(float(y_end)))
            x_end = int(round(float(x_end)))
            cat = int(cat)

            # Clamp to image bounds
            z_start = max(0, min(z_start, D - 1))
            y_start = max(0, min(y_start, H - 1))
            x_start = max(0, min(x_start, W - 1))

            z_end = max(z_start + 1, min(z_end, D))
            ...
            y_start_4 = y_start // 4
            x_start_4 = x_start // 4
            z_end_4 = max(z_start_4 + 1, z_end // 4)
            y_end_4 = max(y_start_4 + 1, y_end // 4)
            x_end_4 = max(x_start_4 + 1, x_end // 4)

            z_start_2 = z_start // 2
            y_start_2 = y_start // 2
            x_start_2 = x_start // 2
            z_end_2 = max(z_start_2 + 1, z_end // 2)
            y_end_2 = max(y_start_2 + 1, y_end // 2)
            x_end_2 = max(x_start_2 + 1, x_end // 2)

            up1 = f_4[b, :, z_start_4:z_end_4, y_start_4:y_end_4, x_start_4:x_end_4,].unsqueeze(0)
            up2 = self.up2(up1)
            skip2 = f_2[b, :, z_start_2:z_end_2, y_start_2:y_end_2, x_start_2:x_end_2,].unsqueeze(0)

            # Sometimes odd-sized boxes cause 1-voxel mismatch after upsampling.
            # Match skip2 to up2 spatial size.
            if skip2.shape[2:] != up2.shape[2:]:
                skip2 = F.interpolate(
                    skip2,
                    size=up2.shape[2:],
                    mode="trilinear",
            ...
            up2 = self.back2(torch.cat((up2, skip2), 1))
            up3 = self.up3(up2)

            im = img[b, :, z_start:z_end, y_start:y_end, x_start:x_end,].unsqueeze(0)

            # Match image crop to up3 spatial size if needed.
            if im.shape[2:] != up3.shape[2:]:
                im = F.interpolate(
                    im,
                    size=up3.shape[2:],
                    mode="trilinear",
            ...
            logits = getattr(self, "logits" + str(cat))(up3)
            logits = logits.squeeze()

            # Match logits back to original detection crop size before placing in full-volume mask
            crop_size = (z_end - z_start, y_end - y_start, x_end - x_start,)

            if logits.shape != crop_size:
            ...
            mask = torch.zeros((D, H, W), device=img.device, dtype=logits.dtype)
            mask[z_start:z_end, y_start:y_end, x_start:x_end] = logits
            mask = mask.unsqueeze(0)
        ...
        if len(out) == 0:
            return torch.zeros((0, D, H, W), device=img.device, dtype=img.dtype)
