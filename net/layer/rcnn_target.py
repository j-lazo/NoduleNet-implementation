...
def make_rcnn_target(cfg, mode, inputs, proposals, truth_boxes, truth_labels, truth_masks):
    ...
    sampled_proposals = [p.view(-1, 8).float() for p in sampled_proposals]
    sampled_labels = [l.view(-1).long() for l in sampled_labels]
    sampled_targets = [t.view(-1, 6).float() for t in sampled_targets]
    sampled_assigns = [np.asarray(a, dtype=np.int32).reshape(-1) for a in sampled_assigns]
    ...
    sampled_assigns = [np.asarray(x, dtype=np.int32).reshape(-1) for x in sampled_assigns]
