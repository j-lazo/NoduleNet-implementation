...
# -----------------------------
def ddp_is_available_and_initialized() -> bool:
    return dist.is_available() and dist.is_initialized()


def get_world_size() -> int:
    return dist.get_world_size() if ddp_is_available_and_initialized() else 1


def get_rank() -> int:
    return dist.get_rank() if ddp_is_available_and_initialized() else 0
...
def setup_distributed(args):
    ...
    Use --multi-gpu auto or --multi-gpu ddp and launch with:
      torchrun --standalone --nproc_per_node=<N> train_nodulenet_experiment.py ...
    """
    requested = args.multi_gpu
    ...
    if requested == 'ddp':
        if world_size_env <= 1:
            raise RuntimeError(
                '--multi-gpu ddp requires launching with torchrun so WORLD_SIZE > 1. '
                'Example: torchrun --standalone --nproc_per_node=2 train_nodulenet_experiment.py ...'
            )
        use_ddp = True
    elif requested == 'auto':
        # Auto means: use DDP if torchrun created multiple processes.
        # Plain python with multiple visible GPUs stays single-process, because auto-spawning
        # is fragile on clusters and with this legacy NoduleNet code.
        use_ddp = world_size_env > 1
    elif requested == 'off':
        use_ddp = False
    else:
        raise ValueError(f'Unknown --multi-gpu value: {requested}')
    ...
    if use_ddp:
        ...
        device = torch.device('cuda', local_rank_env)
        rank = get_rank()
        ...
        rank = 0
        world_size = 1
        if requested == 'auto' and visible_gpus > 1:
            print(
                f'Found {visible_gpus} GPUs, but running single-process. '
                'For multi-GPU training launch with: '
                f'torchrun --standalone --nproc_per_node={visible_gpus} train_nodulenet_experiment.py ...',
                flush=True,
            )

    return use_ddp, device, rank, world_size, local_rank_env


def cleanup_distributed():
    if ddp_is_available_and_initialized():
        dist.barrier()
        dist.destroy_process_group()


def reduce_log_dict(log_dict: dict) -> dict:
    """Average scalar logs across DDP ranks."""
    if not ddp_is_available_and_initialized():
        return log_dict
    reduced = {}
    device = torch.device('cuda', int(os.environ.get('LOCAL_RANK', '0')))
    for k, v in log_dict.items():
        value = 0.0 if (v is None or (isinstance(v, float) and np.isnan(v))) else float(v)
        t = torch.tensor([value], dtype=torch.float32, device=device)
        dist.all_reduce(t, op=dist.ReduceOp.SUM)
        reduced[k] = float((t / get_world_size()).item())
    return reduced


def broadcast_bool(value: bool, device) -> bool:
    if not ddp_is_available_and_initialized():
        return bool(value)
...
def make_loader(dataset, batch_size, shuffle, num_workers, sampler=None, drop_last=False):
    return DataLoader(
        dataset,
...
def set_stage(net, epoch, epoch_rcnn=65, epoch_mask=80):
    ...
    model.use_rcnn = epoch >= epoch_rcnn
    model.use_mask = epoch >= epoch_mask
...
def train_one_epoch(net, loader, optimizer, epoch, device, writer=None):
    ...
    pbar = tqdm(loader, desc=f'Train epoch {epoch}', disable=not show_pbar)
    ...
    for inputs, truth_bboxes, truth_labels, truth_masks, masks, case_ids in pbar:
        ...
        loss, rpn_stat, rcnn_stat, mask_stat = model.loss()

        optimizer.zero_grad(set_to_none=True)
        loss.backward()
        optimizer.step()

        batch_log = collect_losses(model, loss)
        logs.append(batch_log)
        if show_pbar:
            pbar.set_postfix({'loss': f"{batch_log['loss']:.4f}"})
        cleanup_nodulenet_tensors(model)

    epoch_log = reduce_log_dict(average_logs(logs))
...
def validate_one_epoch(net, loader, epoch, device, writer=None):
    ...
    logs = []
    show_pbar = is_main_process()
    pbar = tqdm(loader, desc=f'Val epoch {epoch}', disable=not show_pbar)
...
def save_checkpoint(path: Path, net, optimizer, epoch: int, history: List[dict], args, best_metric: float, split_ids: dict):
    net_to_save = unwrap_model(net)
    state_dict = {k: v.detach().cpu() for k, v in net_to_save.state_dict().items()}
    ckpt = {
        'epoch': epoch,
...
def parse_args():
    ...
    parser.add_argument('--multi-gpu', choices=['auto', 'ddp', 'off'], default='auto',
                        help='auto: use DDP only when launched with torchrun; ddp: require torchrun; off: force single process')
    parser.add_argument('--dist-backend', type=str, default='nccl', help='Distributed backend for DDP')
...
def main():
    ...
    use_ddp, device, rank, world_size, local_rank = setup_distributed(args)
    ...
    train_loader = make_loader(train_dataset, args.batch_size, shuffle=True, num_workers=args.num_workers, sampler=train_sampler)
    val_loader = make_loader(val_dataset, args.batch_size, shuffle=False, num_workers=args.num_workers, sampler=val_sampler)

    # Run final prediction only on rank 0 to avoid duplicate output files.
    ...
    train_writer = SummaryWriter(str(tb_dir / 'train')) if is_main_process() else None
    val_writer = SummaryWriter(str(tb_dir / 'val')) if is_main_process() else None
    ...
    try:
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            model_for_stage = unwrap_model(net)
            if is_main_process():
                print(f"\nEpoch {epoch}/{args.epochs} | lr={lr:.6g} | use_rcnn={model_for_stage.use_rcnn} | use_mask={model_for_stage.use_mask} | world_size={world_size}")
            ...
            row = {
                'epoch': epoch,
                'lr': lr,
                'use_rcnn': bool(unwrap_model(net).use_rcnn),
                'use_mask': bool(unwrap_model(net).use_mask),
            ...
            stop_now = broadcast_bool(stop_now, device)
            if stop_now:
                break
    ...
    # Evaluate/predict with the overall best model on rank 0 only; otherwise every rank would write duplicate files.
    if is_main_process():
        best_path = model_dir / 'best.ckpt'
        model_for_io = unwrap_model(net)
        if best_path.exists():
            ckpt = torch_load_checkpoint(best_path, map_location='cpu')
            model_for_io.load_state_dict(ckpt['state_dict'])
            model_for_io.to(device)

        test_df = predict_test_set(model_for_io, test_loader, test_pairs, out_dir, device, threshold=args.pred_threshold)
